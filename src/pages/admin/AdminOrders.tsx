                     ...
                     <td className="px-6 py-6">
                        {order.status?.toLowerCase() === 'cancelled' ? (
                           <div className="flex flex-col items-start">
                             <div className="bg-rose-50 text-rose-700 border border-rose-200 text-[9px] uppercase tracking-widest font-bold px-3 py-2 rounded-full inline-flex items-center gap-1.5">
                                <XCircle size={12} />
                                CANCELLED
                             </div>
                             <div className="text-[9px] text-rose-500 uppercase tracking-widest font-bold mt-2">
                                Cancelled by {order.cancelledBy === 'user' ? 'User' : 'Admin'}
                             </div>
                           </div>
                        ) : (
                          <select 
                            value={order.status?.toLowerCase() || (order as any).orderStatus?.toLowerCase() || 'pending'}
                                    ...
                                    <div>
                                       ...
                                       <div className="space-y-3">
                                          {order.items.map((item, itemIdx) => (
                                             <div key={`order-details-${order.id}-item-${itemIdx}`} className="flex gap-4 bg-white p-4 border border-zinc-200 group hover:border-black transition-colors">
